...
# Install grub-btrfs
yay -Syu --needed --noconfirm btrfs-progs grub bash gawk \
	inotify-tools grub-btrfs || \
		die "Failed to install required packages with yay."
...
sudo sed -i "/^GRUB_THEME=/s|^GRUB_THEME=.*|$GRUB_THEME_LINE|" \
	/etc/default/grub || die "Failed to set GRUB_THEME line in /etc/default/grub."

sudo grep -qxF "$GRUB_THEME_LINE" /etc/default/grub || \
	echo -e "\n$GRUB_THEME_LINE" | sudo tee -a /etc/default/grub \
	> /dev/null || die "Failed to append GRUB_THEME line to /etc/default/grub."
