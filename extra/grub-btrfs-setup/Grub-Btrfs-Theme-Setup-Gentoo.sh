...
# USE Systemd
echo "app-backup/grub-btrfs systemd" | sudo tee /etc/portage/package.use/grub-btrfs > /dev/null || die "Failed to set USE flag for grub-btrfs."
 
# Allow Unstable Package to be Merged
echo "app-backup/grub-btrfs ~amd64" | sudo tee /etc/portage/package.accept_keywords/grub-btrfs > /dev/null || die "Failed to add grub-btrfs to package.accept_keywords."
