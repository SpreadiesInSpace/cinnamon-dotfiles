...
wget -qO Install-Common.sh "$URL/main/extra/ISO/Install-Common.sh" \
	2>/dev/null || die "Failed to download Install-Common.sh"
...
[ ! -d "/var/log/mount/slackware64" ] && \
	die "Slackware package directory not found at" \
	"/var/log/mount/slackware64."
package_sets=()
dir=""
...
: "${svc:=}"
: "${svc_path:=}"
export drive hostname timezone username rootpasswd svc svc_path userpasswd \
...
wget -q \
	https://gitlab.archlinux.org/archlinux/arch-install-scripts/-/archive/v29/arch-install-scripts-v29.tar.gz \
	-O arch-install-scripts-v29.tar.gz \
	|| die "Failed to download arch-install-scripts source."
wget -q \
	https://slackbuilds.org/slackbuilds/15.0/system/arch-install-scripts.tar.gz \
...
>> /etc/fstab || die "Failed to append essential mounts to fstab."
...
useradd -m -g users \
	-G wheel,audio,video,plugdev,netdev,lp,scanner \
	-s /bin/bash "\$username" || \
