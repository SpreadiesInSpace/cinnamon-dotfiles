...
# Function to emerge packages and run etc-update
emergeAndUpdate() {
    emerge --autounmask-write -qDuN --with-bdeps=y "$@"
    if etc-update --preen | grep -q 'No files found'; then
        echo "No configuration updates needed."
    else
...
#emergeAndUpdate "${desktop_environment[@]}"

# Install Brave
eselect repository enable gentoo-zh
emaint sync -r gentoo-zh
emergeAndUpdate www-client/brave-bin
...
# Install rmlint
emergeAndUpdate -quN dev-build/scons dev-libs/glib
...
emergeAndUpdate "${system_utilities[@]}"
...
emergeAndUpdate "${virtualization_tools[@]}"
