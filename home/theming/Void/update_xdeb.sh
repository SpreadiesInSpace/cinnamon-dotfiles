...
check_and_download_vscodium() {
  ...
  DEB_URL=$(echo "$LATEST_RELEASE" | grep -oP '"browser_download_url": "\K(.*codium_.*_amd64\.deb)(?=")') || die "Failed to find VSCodium .deb URL."

  echo "Latest VSCodium version: $VERSION"

  if command -v codium &>/dev/null; then
    INSTALLED_VERSION=$(codium --no-sandbox --user-data-dir $HOME --version | head -n1 | grep -oP '^\d+(\.\d+)+') || die "Failed to get installed VSCodium version."
    echo "Installed version: $INSTALLED_VERSION"
    if [[ "$INSTALLED_VERSION" == "${VERSION#v}" ]]; then
...
convert_and_install_vscodium() {
  ...
  CODIUM_DEB=$(find . -maxdepth 1 -type f -name 'codium_*_amd64.deb' | sort -V | tail -n1) || die "Failed to find VSCodium .deb package."

  [ -n "$CODIUM_DEB" ] || die "No VSCodium .deb package found."

  echo "Converting $CODIUM_DEB to xbps..."
  ./xdeb -Sedf "$CODIUM_DEB" || die "Failed to convert VSCodium .deb to xbps."
  ...
  if ! command -v codium &>/dev/null; then
    ...
    [ -x /usr/share/codium/bin/codium ] && sudo ln -sf /usr/share/codium/bin/codium /usr/local/bin/codium || die "Failed to symlink codium to /usr/local/bin."
  fi
}
